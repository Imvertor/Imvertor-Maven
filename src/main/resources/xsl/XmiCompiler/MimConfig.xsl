...
<xsl:stylesheet 
    ...
    <xsl:template match="/">
      <xsl:variable name="modelimporttype" select="imf:get-xparm('appinfo/modelimporttype')"/>
      <xsl:variable name="metamodel-owner" select="imf:get-xparm('cli/owner')"/>
      <xsl:variable name="other-specified-version" select="()"/><!-- versie in model volgens een ander metamodel --> 
      <xsl:variable name="cfg" as="element(cfg)">
        <cfg>
          <xsl:choose>
            <xsl:when test="$modelimporttype = 'mim'">
              <xsl:variable name="mim-specified-version" select="/mim:Informatiemodel/mim:MIMVersie"/>
              ...
              <xsl:variable name="metamodel-extension" select="/mim:Informatiemodel/mim:MIMExtensie"/>
              <xsl:variable name="metamodel-extension-version" select="if (imf:boolean(/mim:Informatiemodel/mim:MIMExtensie)) then /mim:Informatiemodel/mim-ext:kenmerken/mim:Kenmerk[@naam = 'MIM extensie versie'] else ()"/>
              <mim-specified-version>{$mim-specified-version}</mim-specified-version>
              <metamodel-version>{$metamodel-version}</metamodel-version>
              <metamodel-extension>{$metamodel-extension}</metamodel-extension>
              <metamodel-extension-version>{$metamodel-extension-version}</metamodel-extension-version>
              <metamodel-name>{if (exists($mim-specified-version)) then 'MIM' else imf:get-xparm('cli/metamodelname')}</metamodel-name>
            </xsl:when>
            <xsl:when test="$modelimporttype = 'xmi'">
              <xsl:variable name="mim-specified-version" select="local:value((//UML:TaggedValue[lower-case(@tag) = ('mim versie','mim version')])[1]/@value)"/>
              <xsl:variable name="metamodel-version" select="local:value(($mim-specified-version,$other-specified-version,imf:get-xparm('cli/metamodelversion')))"/>
      ...
      -->
      <xsl:variable name="toks" select="tokenize($cfg/metamodel-version,'\.')"/>
      <xsl:variable name="minor-version" select="if ($toks[2]) then $toks[1] || '.' || $toks[2] else $cfg/metamodel-version"/>
      <xsl:variable name="major-version" select="if ($toks[2]) then $toks[1] else $cfg/metamodel-version"/>
      
      <xsl:variable name="metamodel-name-and-version" select="if ($cfg/metamodel-name) then local:compact((
        $metamodel-owner,
        $cfg/metamodel-name, 
        $minor-version,
        $cfg/metamodel-extension, 
        $cfg/metamodel-extension-version
        )) else ()"/>
      
      <xsl:variable name="validation-name-and-version" select="if ($cfg/metamodel-name) then local:compact((
        $metamodel-owner,
        $cfg/metamodel-name, 
        $major-version,
        $cfg/metamodel-extension, 
      ...
      -->
      <xsl:sequence select="imf:set-config-string('appinfo','metamodel-name',$cfg/metamodel-name)"/>
      <xsl:sequence select="imf:set-config-string('appinfo','metamodel-minor-version',$minor-version)"/>
      <xsl:sequence select="imf:set-config-string('appinfo','metamodel-specified-version',$cfg/metamodel-version)"/>
      <xsl:sequence select="imf:set-config-string('appinfo','metamodel-major-version',$major-version)"/>
      <xsl:sequence select="imf:set-config-string('appinfo','metamodel-extension',$cfg/metamodel-extension)"/>
      <xsl:sequence select="imf:set-config-string('appinfo','metamodel-extension-version',$cfg/metamodel-extension-version)"/>
