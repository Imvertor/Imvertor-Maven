...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:ep="http://www.imvertor.org/schema/endproduct"
...
servers:
  - url: https://virtserver.swaggerhub.com/King3/King_OA3/3.0</xsl:text>
...
Vervolgens heet het antwoord object 'vergadering'. Deze naamgevingen moeten gebruikt worden in de API beschrijving als pad namen.
        ...
        <xsl:for-each select="ep:message-set/ep:message[ep:type='Vraagbericht' or ep:type='Vrij bericht'][@messagetype = 'request']">
            ...
            <xsl:if test="exists(/ep:message-sets/ep:message-set/ep:message[@servicename = $servicename][@messagetype = 'response'])">
                <xsl:text>&#xa; </xsl:text><xsl:value-of select="concat('/',$kvnamespace,'/',$servicename,':')"/>
            <xsl:text>&#xa;  get:
   tags:
    - </xsl:text><xsl:value-of select="$kvnamespace"/>
                <xsl:text>   
   summary: ''
   operationId: </xsl:text><xsl:value-of select="concat($kvnamespace, '_', $servicename)"/>
            <xsl:text>&#xa;   parameters: </xsl:text>
	...
	De inhoud van deze drie constructs vormen de parameters van het vraagbericht.-->
                   <xsl:for-each select="ep:seq/ep:construct[ep:tech-name= 'gelijk' or ep:tech-name = 'vanaf' or ep:tech-name = 'totEnMet' or ep:tech-name = 'selectie']/ep:type-name">
                    <xsl:variable name="prefix" select="substring-before(., ':')"/>
                    <xsl:variable name="name" select="substring-after(., ':')"/>
                    <xsl:for-each select="/ep:message-sets/ep:message-set/ep:construct[@prefix=$prefix][ep:tech-name=$name]/ep:seq/ep:construct[not(@ismetadata)][@prefix=$prefix]">
                        <xsl:copy-of select="."/>
                    </xsl:for-each>
                </xsl:for-each>
               </xsl:variable> 
    ...
    - in: </xsl:text>
                    <xsl:choose>
                        <xsl:when test="ep:kerngegeven = 'JA'"><xsl:value-of select="'path'"/></xsl:when>
                        <xsl:otherwise><xsl:value-of select="'query'"/></xsl:otherwise>
                    </xsl:choose>
                    <xsl:text>
      name: </xsl:text><xsl:value-of select="ep:tech-name"/><xsl:text>
      description: OK
      required: </xsl:text>
                    <xsl:choose>
                        <xsl:when test="ep:kerngegeven = 'JA'"><xsl:value-of select="'true'"/></xsl:when>
      ...
      schema:
        $ref: '#/components/schemas/</xsl:text>
                     <xsl:choose>
                         <xsl:when test="ends-with(ep:type-name, '-e')">
                             <xsl:value-of select="substring-before(substring-after(ep:type-name, ':'), '-e')"/>
                         </xsl:when>
                         <xsl:otherwise>
       ...
       schema:
        items:
            ...
            <xsl:for-each select="../ep:message[@messagetype = 'response'][@servicename=$servicename]">
                <xsl:text>  $ref: '#/components/schemas/</xsl:text><xsl:value-of select="substring-after(ep:seq/ep:construct[not(@ismetadata)][@prefix=$kvnamespace][starts-with(ep:type-name, concat($kvnamespace, ':'))][1]/ep:type-name, ':')"/><xsl:text>'</xsl:text>
            </xsl:for-each>
            </xsl:if>
        </xsl:for-each>
