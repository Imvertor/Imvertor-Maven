...
<xsl:stylesheet 
	...
	<!-- This variabele defines if pagination applies, it can take the next values:
		 * true()
		 * false()	-->
    <xsl:variable name="pagination" select="true()"/>
    ...
    <xsl:template match="ep:message-sets">
        ...
        <xsl:for-each select="ep:message-set/ep:construct[ep:tech-name = //ep:message/ep:seq/ep:construct/ep:type-name and not(ep:enum)]">
			<xsl:if test="$debugging">
				"//<xsl:value-of select="concat('OAS00100: ',generate-id())"/>": "<xsl:value-of select="ep:name"/>",
			</xsl:if>
            ...
            <!-- Only if json+hal applies, json+hal constructs are generated here. -->
            <xsl:if test="$uitvoerformaat = 'json+hal'">
				<xsl:if test="$debugging">
					"//<xsl:value-of select="concat('OAS00200: ',generate-id())"/>": "<xsl:value-of select="ep:name"/>",
				</xsl:if>
        ...
        <xsl:for-each select="ep:message-set/ep:construct[(ep:tech-name = //ep:message-set/ep:construct/ep:seq/ep:construct/ep:type-name) and not(ep:tech-name = //ep:message/ep:seq/ep:construct/ep:type-name) and not(ep:enum)]">
			<xsl:if test="$debugging">
				"//<xsl:value-of select="concat('OAS00300: ',generate-id())"/>": "<xsl:value-of select="ep:name"/>",
			</xsl:if>
            <xsl:variable name="type-name" select="ep:type-name"/>
        ...
        <xsl:if test="$uitvoerformaat = 'json+hal'">
			...
			<xsl:if test="$debugging">
				"//<xsl:value-of select="concat('OAS00310: ',generate-id())"/>": "<xsl:value-of select="ep:name"/>",
			</xsl:if>
			...
			<xsl:for-each select="ep:message-set/ep:construct[.//ep:construct[@type='association']]">
				...
				<xsl:value-of select="'},'"/>
				<xsl:apply-templates select=".//ep:construct[@type ='association']" mode="_links"/>
				<xsl:value-of select="'}'"/>
				<xsl:value-of select="'}'"/>
			...
			<xsl:if test="$expand">
				...
				<xsl:if test="ep:message-set/ep:construct[.//ep:construct[@type='association']]">,</xsl:if>
				<xsl:if test="$debugging">
					"//<xsl:value-of select="concat('OAS00320: ',generate-id())"/>": "<xsl:value-of select="ep:name"/>",
				</xsl:if>
				...
				<xsl:for-each select="ep:message-set/ep:construct[.//ep:construct[@type='association']]">
					...
					<xsl:value-of select="'&quot;properties&quot;: {'"/>
					<xsl:apply-templates select=".//ep:construct[@type ='association']" mode="embedded"/>
					<xsl:value-of select="'}'"/>
					<xsl:value-of select="'}'"/>
		...
		<!-- The following properties always have to be generated. -->    
		,"foutbericht" : {
		  "type" : "object",
    ...
    <xsl:template match="ep:message">
        ...
        <xsl:value-of select="concat('&quot;msg-', $elementName,'&quot;: {' )"/>
		...
		<xsl:choose>
			<xsl:when test="$pagination and $json-version = '2.0'">
		...
		<xsl:choose>
			...
			<xsl:when test="$uitvoerformaat = 'json+hal'">
				...
				<xsl:value-of select="'&quot;properties&quot; : {'"/>
				<xsl:value-of select="concat('&quot;',translate(ep:seq/ep:construct[@type!='requestclass']/ep:tech-name,'.','_'),'&quot;: {')"/>
				<xsl:value-of select="'&quot;type&quot;: &quot;array&quot;,'"/>
				<xsl:value-of select="'&quot;items&quot;: {'"/>
				<xsl:value-of select="'&quot;$ref&quot;: &quot;#'"/>
				<xsl:choose>
				...
				<xsl:value-of select="concat(ep:seq/ep:construct[@type!='requestclass']/ep:type-name,'_HAL&quot;')"/>
				<xsl:value-of select="'}'"/>
				<xsl:value-of select="'}'"/>
				<xsl:value-of select="'}'"/>
				<xsl:value-of select="'}'"/>
    ...
    <xsl:template name="construct_jsonHAL">
        <xsl:variable name="elementName" select="translate(ep:tech-name,'.','_')"/>

        <xsl:value-of select="concat('&quot;', $elementName,'_HAL&quot;: {' )"/>

		<xsl:value-of select="'&quot;allOf&quot;: ['"/>
		<!-- A HAL type is based on a normal type of the same elementname. -->
	...
	<!-- Embedded types are generated here. -->
    ...
    <xsl:template match="ep:construct[@type='association']" mode="embedded">
        <xsl:variable name="elementName" select="translate(ep:tech-name,'.','_')"/>
