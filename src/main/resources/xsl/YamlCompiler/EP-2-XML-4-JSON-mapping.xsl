...
<?x		<xsl:for-each select="//ep:construct[ep:parameters/ep:parameter[ep:name = 'type' and ep:value = 'association'] and ep:choice]">
		...
		<map>
			<xsl:if test="$json-schemadeclaration = true()">
				<string key="$schema">http://json-schema.org/draft-04/schema#</string>
				<string key="description">Comment describing your JSON Schema</string>
			</xsl:if>
							...
							<xsl:variable name="halEntiteiten">
								...
								<xsl:for-each select="ep:message-set/ep:construct
									...
									<xsl:if test="contains( ep:parameters/ep:parameter[ep:name='berichtcode']/ep:value,'Gc')">
										...
										<map key="{concat($tech-name,'HalCollectie')}">
												...
												<map key="_links">	
													<xsl:choose>
														<xsl:when test="$pagination='true'">
															<string key="$ref">
																<xsl:value-of select="concat($standard-json-components-url,'HalPaginationLinks')"/>
															</string>
														</xsl:when>
														<xsl:otherwise>
															<string key="$ref">
																<xsl:value-of select="concat($standard-json-components-url,'HalCollectionLinks')"/>
															</string>
														</xsl:otherwise>
													</xsl:choose>
												</map>
												<map key="_embedded">
													...
													<map key="properties">
														<map>
															...
															<map key="items">
																<string key="$ref">
																	<xsl:value-of select="concat($json-topstructure,'/',translate($tech-name,'.','_'),'Hal')" />
																</string>
															</map>
														</map>																									
													</map>												
									...
									<xsl:if test="$debugging">
										<map key="--------------Debuglocatie-01000">
						                	<string key="XPath"><xsl:sequence select="imf:xpath-string(.)"/></string>
										</map>
									</xsl:if>
			...
			<map>
				<xsl:choose>
					<xsl:when test="$json-version = '2.0'">
						<xsl:attribute name="key">definitions"</xsl:attribute>
						<xsl:copy-of select="$json-components/map"/>
					</xsl:when>
					<xsl:when test="$json-version = '3.0'">
						<xsl:attribute name="key">components"</xsl:attribute>
						<map key="schemas">
							<xsl:copy-of select="$json-components/map"/>
						</map>
					</xsl:when>
	...
	<xsl:function name="imf:createHalComponent">
		...
		<map key="{concat($elementName,'Hal')}">
			<array key="allOf">
				<string key="$ref"><xsl:value-of select="concat($json-topstructure,'/',$elementName)"/></string>
				<map>
					<string key="type">object</string>
