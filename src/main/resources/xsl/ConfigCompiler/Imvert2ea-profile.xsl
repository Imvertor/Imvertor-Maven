...
<xsl:stylesheet 
    ...
    <xsl:param name="nature" select="tokenize(imf:get-xparm('cli/metamodelnature'),';')" as="xs:string*"/>
    ...
    <xsl:template match="/config">
        ...
        <xsl:variable name="measures" select="$visuals/measure[@nature = $nature]"/>
        <xsl:variable name="stereos" select="$visuals/stereo[@nature = $nature]"/>
        <xsl:variable name="categories" select="$visuals/category[@nature = $nature]"/>
        ...
        <xsl:comment>This file may be imported into EA and aligns fully with the metamodel named {imf:get-xparm('appinfo/metamodel')} and nature {imf:get-xparm('cli/metamodelnature')} </xsl:comment>
            ...
            <UMLProfile profiletype="uml2">
                ...
                <Content>
                    <Stereotypes>
                        ...
                        <xsl:for-each select="$metamodel/stereotypes/stereo[not(@origin='system') and empty(@cross-meta) and @nature = $nature]/name">
                            <xsl:sort select="."/>
