...
<xsl:stylesheet 
    ...
    <xsl:variable name="profile-spec" select="tokenize(imf:get-config-string('cli','createeaprofile','UNKNOWN-ID;UNKNOWN-NAME;UNKNOWN-RELEASE'),';')"/>
    <xsl:variable name="profile-id" select="$profile-spec[1]"/>
    <xsl:variable name="profile-name" select="$profile-spec[2]"/>
    <xsl:variable name="profile-release" select="$profile-spec[3]"/>
    
    <xsl:template match="/config">
        
        <xsl:variable name="tagged-values" select="tagset/tagged-values"/>
        
        <xsl:variable name="profile-id" select="imf:extract(upper-case($profile-spec[1]),'[A-Z0-9]+')"/>
        <xsl:variable name="profile-name" select="$profile-spec[2]"/>
        <xsl:variable name="profile-version" select="$profile-spec[3]"/>
