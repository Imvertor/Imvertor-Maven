...
<xsl:stylesheet 
    ...
    <xsl:variable name="diff-url" select="imf:file-to-url($diff-filepath)"/>
    <xsl:variable name="ctrl-url" select="imf:file-to-url($ctrl-filepath)"/>
    <xsl:variable name="test-url" select="imf:file-to-url($test-filepath)"/>
    ...
    <!-- 
        remove all diffs that are not considered important; they are linked to some other diff that clarifies a difference by itself.
    -->
    <xsl:template match="diff" mode="clean-adds">
        <xsl:variable name="copy" as="xs:boolean">
            <xsl:choose>
                <xsl:when test="@desc='sequence of child nodes'">
                    <xsl:sequence select="false()"/>
                </xsl:when>
                <xsl:when test="test/@path = 'null'">
                    <xsl:sequence select="true()"/>
                </xsl:when>
                <xsl:when test="ctrl/@path = 'null'">
                    <xsl:sequence select="true()"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:sequence select="true()"/>
