...
<xsl:stylesheet 
    ...
    <xsl:template match="/">
       ...
       <!-- and record the supplier(s) of the application itself -->
        ...
        <xsl:for-each select="$supplier-application-info">
            <xsl:sequence select="imf:set-config-string('appinfo','supplier-etc-system-imvert-path',@system-path,false())"/>
            <xsl:sequence select="imf:set-config-string('appinfo','supplier-etc-model-imvert-path',@model-path,false())"/>
        </xsl:for-each>
    ...
    <xsl:function name="imf:get-full-derivation-sub" as="element()*">
        ...
        <!-- 
                Determine which package(s) is/are the supplier.
        ...
        <xsl:for-each select="$supplier-info">
            ...
            <xsl:choose>
                ...
                <xsl:when test="empty($package/imvert:supplier) and not(imf:boolean($package/imvert:derived))">
                    <!-- okay, skip. This occurs only for base packages, that have no supplier (CDMKAD, SIM, ...) -->
                </xsl:when>
                
                <xsl:when test="@application != '' and @project = ''">
                    <xsl:sequence select="imf:report-error($package,true(),'No supplier project specified for supplier [1]', @application)"/>
                </xsl:when>
                
                <xsl:when test="@application != '' and @release = ''">
                ...
                <xsl:when test="@application">
                    ...
                    <xsl:variable name="supplier-document" select="imf:document(@system-path)"/>
                    <xsl:variable name="supplier-mapped-name" select="@package-name"/>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <imvert:supplier id="{imf:get-supplier-id(@project,@application,@release)}" application="{@application}" project="{@project}" release="{@release}" level="{$level}">
