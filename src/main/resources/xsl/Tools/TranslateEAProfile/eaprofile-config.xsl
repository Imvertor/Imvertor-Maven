<xsl:stylesheet 
    ...
    <xsl:template match="/worksheets/worksheet[@name = 'EA profile']">
                                ...
                                <condition subject="{substring-before(current-grouping-key(),'=')}" object="{substring-after(current-grouping-key(),'=')}">
                                    <xsl:for-each-group select="current-group()" group-by="cell[@nr = '4']">
                                        <attribute name="{current-grouping-key()}" token="{cell[@nr = '5']}">
                                            <value lang="NL"><xsl:value-of select="current-group()/cell[@nr = '6']"/></value>
                                            <value lang="EN"><xsl:value-of select="current-group()/cell[@nr = '7']"/></value>
                                        </attribute>
                                    </xsl:for-each-group>
                                </condition>
                            </xsl:for-each-group>
