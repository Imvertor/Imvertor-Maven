...
<xsl:stylesheet 
	...
	<xsl:template match="ep:message-set">
		...
		<xs:schema targetNamespace="{$message-set-namespaceIdentifier}" elementFormDefault="qualified" attributeFormDefault="unqualified" version="{concat(ep:patch-number,'-',ep:release)}">
			...
			<xsl:variable name="namespaces2bImported">
				<ep:namespaces>
					...
					<xsl:if test="@KV-namespace = 'yes'">
						<xsl:for-each select=".//ep:constructRef[@prefix and not(@prefix = $message-set-prefix) and not(@prefix = $StUF-prefix) and not(@prefix = '$actualPrefix')] | .//ep:construct[@prefix and not(@prefix = $message-set-prefix) and not(@prefix = $StUF-prefix) and not(@prefix = '$ACTUALPREFIX')]">
							<xsl:variable name="prefix" select="@prefix"/>
							<ep:namespace identifier="{$namespaces//ep:namespace[@prefix = $prefix]}" prefix="{$prefix}"/>
						</xsl:for-each>
					</xsl:if>
				</ep:namespaces>
			</xsl:variable>
			...
			<xsl:apply-templates select="ep:construct[empty(@ismetadata) and empty(ep:data-type) and empty(ep:type-name)]" mode="complexType"/>
			...
			<xsl:sequence select="imf:create-debug-comment('ROME: simpleType extending complexTypes',$debugging)"/>
			<xsl:apply-templates select="ep:construct[substring-after(ep:type-name,':') = //ep:message-set/ep:construct[@isdatatype and @prefix=$StUF-prefix]/ep:tech-name and @prefix=$StUF-prefix]" mode="simpleContentComplexType"/>
	...
	<xsl:template match="ep:construct">
		...
		<xsl:variable name="type-name">
			<xsl:choose>
				<xsl:when test="contains(ep:type-name,':')"><xsl:value-of select="ep:type-name"/></xsl:when>
				<xsl:otherwise><xsl:value-of select="concat(@prefix,':',ep:type-name)"/></xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xs:element>
			...
			<xsl:choose>
				<xsl:when test="ep:type-name and not(ep:enum)">
					...
					<xsl:sequence select="imf:create-debug-comment('Debuglocation 5005',$debugging)"/>
					<xsl:if test="ep:documentation">
						<xs:annotation>
							<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
						</xs:annotation>
...
<!-- ROME: Hieronder wordt een id gegenereerd. Dat is echter eigenlijk niet gewenst omdat daarbij de naam van de simpleType na elke generatie slag anders kan zijn.
		   Dat zou betekenen dat leveranciers steeds hun gegenereerde code moeten aanpassen. We moeten dus een manier zien te vinden die toekomstvaster is. -->
				<xsl:when test="ep:type-name and ep:enum">
					...
					<xsl:attribute name="minOccurs" select="ep:min-occurs"/>
					<xsl:attribute name="maxOccurs" select="ep:max-occurs"/>					
					<xsl:sequence select="imf:create-debug-comment('Debuglocation 5006',$debugging)"/>
					<xsl:if test="ep:documentation">
						<xs:annotation>
							<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
						</xs:annotation>
				...
				<xsl:when test="ep:data-type and
					...
					<xsl:attribute name="type">
						<xsl:choose>
							...
							<xsl:when test="$data-type = 'scalar-datetime' and ep:type-modifier = '?'">
				...
				<xsl:otherwise>
					...
					<xsl:choose>
						...
						<xsl:when test="(ep:length or ep:max-length or ep:min-length or ep:max-value or ep:min-value or ep:fraction-digits or ep:formeel-patroon or ep:regels or ep:enum) and ep:type-name and .//ep:construct[@ismetadata]">
							<xsl:sequence select="imf:create-debug-comment('Debuglocation 5007',$debugging)"/>
							<xsl:if test="ep:documentation">
								<xs:annotation>
									<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
								</xs:annotation>
						...
						<xsl:when test="(ep:length or ep:max-length or ep:min-length or ep:max-value or ep:min-value or ep:fraction-digits or ep:formeel-patroon or ep:regels or ep:enum) and ep:type-name">
							<xsl:sequence select="imf:create-debug-comment('Debuglocation 5008',$debugging)"/>
							<xsl:if test="ep:documentation">
								<xs:annotation>
									<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
								</xs:annotation>
						...
						<xsl:when test="$type-name and .//ep:construct[@ismetadata]">
							<xsl:sequence select="imf:create-debug-comment('Debuglocation 5009',$debugging)"/>
							<xsl:if test="ep:documentation">
								<xs:annotation>
									<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
								</xs:annotation>
						...
						<xsl:when test="ep:type-name">
							<xsl:sequence select="imf:create-debug-comment('Debuglocation 5010',$debugging)"/>
							<xsl:if test="ep:documentation">
								<xs:annotation>
									<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
								</xs:annotation>
						...
						<xsl:when test=".//ep:construct">
							<xsl:sequence select="imf:create-debug-comment('Debuglocation 5011',$debugging)"/>
							<xsl:if test="ep:documentation">
								<xs:annotation>
									<xs:documentation><xsl:value-of select="ep:documentation"/></xs:documentation>
								</xs:annotation>
		...
		<xsl:if test="ep:seq/ep:* or ep:choice/ep:*">
			<xs:complexType>
				...
				<xsl:choose>
					...
					<xsl:otherwise>
						<xsl:sequence select="imf:create-debug-comment('Debuglocation 5014',$debugging)"/>
						<xsl:apply-templates select="ep:seq | ep:choice"/>
						<xsl:apply-templates select="ep:seq" mode="generateAttributes"/>						
					</xsl:otherwise>
				</xsl:choose>
			...
			<xs:restriction>
				...
				<xsl:if test="ep:length">
					<xsl:choose>
						...
						<xsl:when test="ep:data-type = 'scalar-integer' or ep:data-type = 'scalar-decimal'">
							<xs:totalDigits value="{ep:length}" />
						</xsl:when>
					</xsl:choose>
				</xsl:if>
				...
				<xsl:if test="ep:min-value and (ep:data-type = 'scalar-integer' or ep:data-type = 'scalar-decimal' or ep:data-type = 'scalar-date')">
					<xs:minInclusive value="{ep:min-value}" />
				</xsl:if>
				<xsl:if test="ep:max-value and (ep:data-type = 'scalar-integer' or ep:data-type = 'scalar-decimal' or ep:data-type = 'scalar-date')">
					<xs:maxInclusive value="{ep:max-value}" />
				</xsl:if>
				<xsl:if test="ep:fraction-digits">
					<xs:fractionDigits value="{ep:fraction-digits}" />
				...
				<xsl:if test="ep:formeel-patroon and (ep:data-type = 'scalar-string' or ep:data-type = 'scalar-date' or ep:data-type = 'scalar-integer' or ep:data-type = 'scalar-decimal' or ep:data-type = 'scalar-boolean')">
					<xs:pattern value="{ep:formeel-patroon}" />
				</xsl:if>								
			</xs:restriction>
		</xs:simpleType>
			...
			<xs:element>
				<xsl:choose>
					<xsl:when test="ep:href and (ep:data-type or ep:type-name)">
						<xsl:attribute name="name" select="ep:tech-name"/>
						<?x xsl:variable name="actualPrefix">
							<xsl:choose>
						...
						<xsl:attribute name="type" select="concat($actualPrefix,':',ep:href)"/ x?>					
	...
	<?x xsl:template match="ep:construct" mode="createSimpleTypes">
		<xsl:variable name="id" select="substring-before(substring-after(ep:id,'{'),'}')"/>
