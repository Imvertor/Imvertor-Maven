...
<!-- Robert Melskens	2017-06-09	This stylesheet generates a rough EP file structure based on the embellish 
									file of a BSM EAP file. This rough structure will be used in the next step
									for creating the final EP file structure. -->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="stylesheet">Imvert2XSD-KING-create-openapi-endproduct-rough-structure</xsl:variable>
	<xsl:variable name="stylesheet-version">$Id: Imvert2XSD-KING-create-OpenAPI-endproduct-rough-structure.xsl 1
		2018-04-16 13:32:00Z RobertMelskens $</xsl:variable>
	...
	<xsl:variable name="rough-messages">
		...
		<!-- The 'Berichtstructuren' package isn't of any importance at OpenAPI interface creation. --> 
		<!-- TODO: Is het nog wel van belang om dan te checken op de alias van de Berichtstructuren package of mogen we er gewoon vanuit gaan dat dat package niet gebruikt wordt in de OAS context? -->
		<ep:rough-messages>
	...
	<xsl:template
		match="imvert:package" mode="create-rough-message-structure">
		...
		<!-- TODO: De vraag is of bij OpenAPI generatie nog wel het onderscheid tussen de verschillende berichttypen gemaakt moet kunnen worden. -->			

		<!-- The following apply-templates processes all classes representing a messagetype. -->
<!--		<xsl:variable name="OAS-messages">
			<xsl:for-each select="imvert:class[(imvert:stereotype/@id = 'stereotype-name-vraagberichttype')]">
				<xsl:variable name="berichtcode" select="imf:get-tagged-value(.,'##CFG-TV-BERICHTCODE')"/>
				<xsl:value-of select="concat($berichtcode,';')"/>
			</xsl:for-each>
		</xsl:variable>
		<xsl:if test="not(contains($OAS-messages,'Gr')) and not(contains($OAS-messages,'Gc'))">
			<xsl:variable name="msg" select="'In dit koppelvlak zijn geen OAS berichten gedefinieerd.'" as="xs:string"/>
			<xsl:sequence select="imf:msg('WARNING',$msg)"/>
		</xsl:if> -->
		<xsl:apply-templates select="imvert:class[(imvert:stereotype/@id = ('stereotype-name-vraagberichttype',
			'stereotype-name-antwoordberichttype',
			'stereotype-name-kennisgevingberichttype',
			'stereotype-name-synchronisatieberichttype',
			'stereotype-name-vrijberichttype'))]" mode="create-rough-messages"/>
	...
	<!-- This template transforms an 'imvert:association' element of stereotype 'entiteitrelatie' to an 'ep:construct' element.. -->
	<xsl:template match="imvert:association[imvert:stereotype/@id = ('stereotype-name-entiteitrelatie')]"
		...
		<xsl:sequence select="imf:create-debug-comment('debug:start A00020 /debug:start',$debugging)"/>
	...
	<xsl:template match="imvert:class" mode="create-rough-message-content">
		...
		<xsl:sequence select="imf:create-debug-comment('debug:start A00030 /debug:start',$debugging)"/>
		...
		<xsl:choose>
			<xsl:when test="not(contains($id-trail, concat('#', $id, '#')))">
				<xsl:choose>
					<!-- If a supertype is refered to as an association the related subtypes are placed as a construct in a sequence within that relation. -->
					<xsl:when test="$proces-type = 'as-normal' and $packages//imvert:class[imvert:supertype/imvert:type-id = $id]">
						<xsl:apply-templates select="$packages//imvert:class[imvert:supertype/imvert:type-id = $id]"
							mode="create-rough-message-content">
					...
					<xsl:when test="$proces-type = 'as-normal'">
						<xsl:sequence select="imf:create-debug-comment('debug:start A00030a /debug:start',$debugging)"/>
						<ep:construct>
							<xsl:choose>
							...
							<xsl:sequence
								select="imf:create-output-element('ep:id', $id)"/>	
							<xsl:sequence
								select="imf:create-output-element('ep:id-refering-association', $id-refering-association)"/>
							
							<!-- TODO: Klopt onderstaande nog wel. Attributes worden nu volgens mij ook geprocessed als het om een attribute gaat dat verwijst naar een tabel entiteit. -->

							<!-- The following takes care of ignoring the processing of the attributes 
									 belonging to the current class. Attributes aren't important for the rough structure but need to be processed here to determine 
									 the processtype of the classes containing the attributes. -->
							...
							<xsl:apply-templates select="imvert:associations/imvert:association" mode="create-rough-message-content">
								<xsl:with-param name="id-trail"	select="concat('#', $id, '#', $id-trail)"/>
							</xsl:apply-templates>
						</ep:construct>
					</xsl:when>
	...
	<xsl:template match="imvert:supertype" mode="create-rough-message-content">
		...
		<xsl:sequence select="imf:create-debug-comment('debug:start A00070 /debug:start',$debugging)"/>
	...
	<xsl:template match="imvert:association" mode="create-rough-message-content">
		...
		<xsl:sequence select="imf:create-debug-comment('debug:start A00080 /debug:start',$debugging)"/>
		...
		<ep:construct type="association">
			...
			<xsl:if test="$debugging">
				<xsl:attribute name="package" select="ancestor::imvert:package/imvert:name"/>
			</xsl:if>
			...
			<xsl:apply-templates select="key('class',$type-id)" mode="create-rough-message-content">
				<xsl:with-param name="id-trail" select="$id-trail"/>
				<!-- Within the construct refering to a class we need to be able to trace which association refered to that class since
					 more than one association can. -->
				<xsl:with-param name="id-refering-association" select="imvert:id"/>
			</xsl:apply-templates>
		</ep:construct>
	...
	<!-- This template generates the structure of an associationconstruct on an associationconstruct. -->
	<xsl:template match="imvert:association-class" mode="create-rough-message-content">
		<xsl:param name="id-trail"/>
		
		<xsl:variable name="type-id" select="imvert:type-id"/>
		
		<xsl:sequence select="imf:create-debug-comment('debug:start A00120 /debug:start',$debugging)"/>
