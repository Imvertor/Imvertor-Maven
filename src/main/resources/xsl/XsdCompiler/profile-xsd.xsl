    ...
    <xsl:template match="/">
        ...
        <xsl:variable name="schema" as="element()">
            <schema 
                ...
                <annotation>
                    <documentation><xsl:value-of select="$config-name"/> - Generated by Imvertor <xsl:value-of select="imf:get-xparm('run/version')"/> at <xsl:value-of select="imf:get-xparm('run/start')"/> for model "<xsl:value-of select="imf:get-xparm('appinfo/subpath')"/>"</documentation>
