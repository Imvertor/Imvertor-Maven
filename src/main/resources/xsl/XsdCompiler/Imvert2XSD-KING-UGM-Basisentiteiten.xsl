...
<xsl:stylesheet 
        ...
        <xsl:variable name="schema" as="element()">
            <xs:schema 
               ...
               <xs:annotation>
                ...
                <xsl:apply-templates select="//imvert:class[imf:get-stereotype(.) = imf:get-config-stereotypes('stereotype-name-composite')]" mode="gegevensgroeptype"/>
                
                <xs:annotation>
                    <xs:documentation>(Referentielijsten)</xs:documentation>
                </xs:annotation>
                <xsl:apply-templates select="//imvert:class[imf:get-stereotype(.) = imf:get-config-stereotypes('stereotype-name-referentielijst')]" mode="referentielijst"/>
                 ?>           
                
                <xs:annotation>
                    <xs:documentation>(Basistypen)</xs:documentation>
                </xs:annotation>
                <xsl:apply-templates select="//imvert:class[imf:get-stereotype(.) = imf:get-config-stereotypes('stereotype-name-objecttype')]" mode="objecttype"/>
                
                <?x
                <xs:annotation>
                    <xs:documentation>(Kerngegevens)</xs:documentation>
                </xs:annotation>
                <xsl:apply-templates select="//imvert:class[imf:get-stereotype(.) = imf:get-config-stereotypes('stereotype-name-objecttype')]" mode="kerngegevens"/>
                ...
                <xsl:for-each-group 
                    select="//imvert:attribute[imf:get-typestruct(.)[3] = '']" 
                    group-by="imf:get-namestruct(.)[3]">
    ...
    <xsl:template match="imvert:class" mode="gegevensgroeptype">
        ...
        <xs:complexType name="{$name-struct[3]}">
            ...
            <xs:sequence>
                ...
                <xsl:apply-templates select="imvert:attributes/imvert:attribute" mode="local-attribute"/>
                <!-- TODO ook bij gegevensgroeptypen?? --> 
                <xsl:comment select="'(Associations)'"/>
                <xsl:apply-templates select="imvert:associations/imvert:association" mode="local-association">
    ...
    <xsl:template match="imvert:class" mode="objecttype">
        <xsl:variable name="name-struct" select="imf:get-namestruct(.)"/>
    ...
    <!-- Groepsattribuutsoort -->
    <xsl:template match="imvert:class" mode="referentielijst">
        <xsl:variable name="name-struct" select="imf:get-namestruct(.)"/>
        <xsl:variable name="id" select="imf:get-id(.)"/>
    ...
    <xsl:template match="imvert:class" mode="kerngegevens">
        <xsl:variable name="name-struct" select="imf:get-namestruct(.)"/>
    ...
    <!-- Attribuutsoort -->
    <xsl:template match="imvert:attribute" mode="local-attribute">
        ...
        <xsl:sequence select="imf:msg(.,'DEBUG', 'Local attribute Complex Typestruct [1]', $type-struct[1])"/>
        
        <xsl:variable name="typeref">
            <xsl:choose>
                <xsl:when test="$type-struct[1] = 'scalar-indic'">
                    <xsl:value-of select="'bg:Indicatie-e'"/>
                </xsl:when>
                <xsl:when test="$type-struct[1] = 'scalar-uri'">
                ...
                <xsl:when test="$type-struct[2] = 'OnvolledigeDatum'">
                    <!-- TODO onvolledigeDatum -->
                    <xsl:value-of select="'StUF:DatumMetIndicator'"/>
                </xsl:when>
                <xsl:when test="$type-struct[1] = 'scalar-integer'">
                    <xsl:value-of select="concat($prefix, ':', imf:capitalize($name-struct[3]), '-e')"/>
                </xsl:when>
                <xsl:when test="$type-struct[1] = 'scalar-string'">
                    <xsl:value-of select="concat($prefix, ':', imf:capitalize($name-struct[3]), '-e')"/>
                </xsl:when>
                <xsl:when test="$type-struct[1] = 'scalar-letter'">
                    <xsl:value-of select="concat($prefix, ':', imf:capitalize($name-struct[3]), '-e')"/>
                </xsl:when>
                <xsl:when test="$type-struct[1] = 'scalar-date'">
                    <xsl:value-of select="concat($prefix, ':', imf:capitalize($name-struct[3]), '-e')"/>
                </xsl:when>
                <xsl:when test="$type-struct[1] = 'scalar-postcode'">
                    <xsl:value-of select="concat($prefix, ':', imf:capitalize($name-struct[3]), '-e')"/>
                </xsl:when>
                <xsl:when test="imf:get-stereotype($type) = imf:get-config-stereotypes('stereotype-name-referentielijst')"> <!-- a reference table -->
                    <!-- 
                    ...
                    -->
                    <xsl:variable name="attribute" select="$type//imvert:attribute[imf:get-namestruct(.)[1] = $name-struct[1]]"/>
                    <xsl:variable name="name-struct-att" select="imf:get-namestruct($attribute)"/>
                    <xsl:if test="empty($attribute)">
                        <xsl:value-of select="imf:msg(.,'ERROR','No attribute with same name in reference list: [1]', $name-struct[1])"/>
                    </xsl:if>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="imf:msg(.,'ERROR','Cannot handle the local attribute type: [1]', $type-struct[1])"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template match="imvert:association" mode="local-association">
    ...
    <xsl:template match="imvert:attribute" mode="declare-attribute-type">
        <xsl:variable name="name-struct" select="imf:get-namestruct(.)"/>
        <xsl:variable name="type-struct" select="imf:get-typestruct(.)"/>
        ...
        <xsl:if test="not($type-struct[1] = ('scalr-indic','scalar-uri')) and not($type-struct[2] = ('OnvolledigeDatum')) and not($type-stereotype = imf:get-config-stereotypes('stereotype-name-composite'))">
            <!--TODO OnvolledigeDatum -->
    ...
    <xsl:template match="imvert:association" mode="declare-association-type">
        <xsl:variable name="name-struct" select="imf:get-namestruct(.)"/>
        <xsl:variable name="type-struct" select="imf:get-typestruct(.)"/>
    ...
    <xsl:template match="imvert:attribute" mode="declare-attribute-simpletype">
        <xsl:variable name="name-struct" select="imf:get-namestruct(.)"/>
        <xsl:variable name="type-struct" select="imf:get-typestruct(.)"/>
        ...
        <xsl:if test="not($type-struct[1] = ('scalar-indic','scalar-uri')) and not($type-struct[2] = ('OnvolledigeDatum'))">
            <!-- TODO OnvolledigeDatum -->
            <xsl:comment select="concat('Attribuut type (simple) ',$name-struct[4])"/>
            <xs:simpleType name="{imf:capitalize($name-struct[3])}">
                <xsl:choose>
                    <xsl:when test="$type-struct[1] = ''">
                    ...
                    <xsl:otherwise>
                        <xsl:value-of select="imf:msg(.,'ERROR','Cannot handle the simple attribute type: [1]', $type-struct[1])"/>
                    </xsl:otherwise>                
                </xsl:choose>
            </xs:simpleType>
        </xsl:if>
    ...
    <xsl:template match="imvert:attribute" mode="declare-attribute-enumeratie">
    ...
    <xsl:function name="imf:get-namestruct" as="xs:string*">
        ...
        <xsl:if test="exists($this)">
            ...
            <xsl:variable name="original" select="$this/imvert:name/@original"/>
            ...
            <xsl:variable name="compiled-name">
                <xsl:choose>
                    ...
                    <xsl:when test="$type = 'attribute' and $stereotype = imf:get-config-stereotypes('stereotype-name-attribuutsoort') and normalize-space($alias)">
                        <xsl:value-of select="$alias"/>
                    </xsl:when>
                    <xsl:when test="$type = 'attribute' and $stereotype = imf:get-config-stereotypes('stereotype-name-attribuutsoort')">
            ...
            <xsl:variable name="source-alias" select="$this/imvert:source-alias"/>
            <xsl:variable name="target-alias" select="$this/imvert:target-alias"/>
    ...
    <xsl:function name="imf:get-typestruct" as="xs:string*">
        <xsl:param name="this"/>
        <xsl:variable name="type" select="($this/imvert:type,$this)[1]"/>
        <xsl:variable name="typename" select="$type/imvert:type-name"/>
        <xsl:variable name="baretype" select="$type/imvert:baretype"/>
        <xsl:variable name="typeid" select="$type/imvert:type-id"/>
        <xsl:variable name="pack" select="$type/imvert:type-package"/>
    ...
    <xsl:function name="imf:get-stereotype" as="xs:string*">
        <xsl:param name="this"/>
        <xsl:value-of select="$this/imvert:stereotype"/>
    </xsl:function>
        ...
        <xsl:variable name="selected-attributes" as="node()*">
            <xsl:choose>
                ...
                <xsl:when test="contains($tv,'in combinatie met')">
                    ...
                    <xsl:variable name="related-id" select="imf:get-id($document//imvert:class[.//imvert:originalName = $selected-names[last()]][1])"/>
                    <xsl:variable name="relation" select="$class//imvert:association[.//imvert:type-id = $related-id]"/>
    ...
    <xsl:function name="imf:get-kerngegevens-attributes" as="element()*">
        ...
        <xsl:variable name="att" select="$class//imvert:attribute[imvert:name/@original = $attspec]"/>
    ...
    <xsl:function name="imf:get-relation-suffix" as="xs:string+">
        ...
        <xsl:value-of select="if (count($this/../imvert:association[imvert:target-alias = $targetAlias]) gt 1) then imf:capitalize(imf:get-namestruct($class)[1]) else ''"/>
