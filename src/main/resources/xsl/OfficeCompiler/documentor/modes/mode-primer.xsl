    ...
    <xsl:template match="a[@href and not(starts-with(@href,'#'))]" mode="pack:mode-primer">
        <xsl:choose>
            <xsl:when test="starts-with(@href,'https://docs.geostandaarden.nl/mim/mim/')">
                <xsl:variable name="href" select="if ($props[@key = 'MIM version'] = '1.1') then 'https://docs.geostandaarden.nl/mim/vv-st-mim-20200225/' else 'https://docs.geostandaarden.nl/mim/def-st-mim-20220217/'"/>
