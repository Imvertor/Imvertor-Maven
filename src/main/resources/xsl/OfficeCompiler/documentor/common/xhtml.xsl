        ...
        <figure class="image image-type-{(@metadata-type,'default')[1]}">
            <xsl:choose>
                <xsl:when test="raw">
                    ...
                    <xsl:variable name="hash" select="local:calculate-hash($raw[2])"/>
                    ...
                    <xsl:variable name="img-path" select="$module-work-folder-path || '/img-store/' || $img-name"/>
                    
                    <!-- save there -->
                    <xsl:try>
                        <xsl:sequence select="file:write-binary($img-path,xs:base64Binary($raw[2]))"/> 
