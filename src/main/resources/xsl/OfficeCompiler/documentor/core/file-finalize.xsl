<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <xsl:import href="../common/common-language.xsl"/>
    ...
    <xsl:template match="/"> <!-- een <document> -->
        ...
        <document name="{$msword-file-subpath}" auto="auto">
    ...
    <xsl:template match="div[@data-custom-style = 'plaatje']">
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <error loc="{$msword-file-subpath}">Plaatje zonder bron, in: {(preceding::title)[1]}</error>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="a[@role = 'doc-noteref']" priority="20">
        ...
        <xsl:choose>
            <xsl:when test="empty($note)">
                <error loc="{$msword-file-subpath}">Deze noot bestaat niet: {$name}</error>
            </xsl:when>
            <xsl:otherwise>
                <note>
    ...
    <xsl:template match="table">
        ...
        <xsl:if test="exists($cols-specified) and (count($cols-specified) ne count($cols-found))">
            <error loc="{$msword-file-subpath}">Aantal kolommen in <b>table: matrix</b> specificatie en kolommen aangetroffen is niet hetzelfde</error>
        </xsl:if>
