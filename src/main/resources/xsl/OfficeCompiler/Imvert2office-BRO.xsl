...
<xsl:stylesheet 
   ...
   <xsl:variable name="quot"><!--'--></xsl:variable>
    ...
    <xsl:template match="/imvert:packages">
        <xsl:call-template name="sample-section"/>
        
        <?x
        <!-- deze header is eventjes toegevoegd om het resultaat te inspecteren. -->
        <html lang="nl">
            <head>
                <meta content="text/html; charset=utf-8" http-equiv="content-type" />
                <meta content="width=device-width,initial-scale=1" name="viewport" />
                <title>BRO gegevenscatalogus [registratieobject]</title>
                ...
                <script class='remove'>
                    ...
                    logos: [
                    {
                    src: "http://www.geonovum.nl/sites/all/themes/insiders/logo.png",
            ...
            <body>
                <!-- dit is bogus, dit moet worden gevuld met de echte info. -->
                <xsl:call-template name="sample-section"/>
            </body>
        </html>
        ?>
    ...
    <xsl:template name="sample-section">
        <!-- HET TE GENEREREN DEEL BEGINT HIER -->
        <h2>Catalogus</h2>
        <section id="catalog">
            <h3>Technische catalogus info</h3>
            <section id="techinfo">
                <p>
                    <xsl:value-of select="concat($application-package-release-name, ' | ', $imvertor-version, ' | ', $generation-date)"/>
                </p>
                <p>
                    Er zijn <xsl:value-of select="count(/imvert:packages//imvert:class)"/> classes.
                </p>
            </section>
        ...
        <!-- HET TE GENEREREN DEEL EINDIGT HIER -->
    </xsl:template>
    
    <xsl:template match="imvert:package[exists(imvert:name/@original)]">
        
        <h3>Beschrijving van de featureTypes (package <xsl:value-of select="imvert:name/@original"/>)</h3>
        <section id="classes">
            <!-- De feature types en gegevensgroeptypen -->
            <xsl:apply-templates select="imvert:class[imvert:stereotype/@id = ('stereotype-name-objecttype')]"/>
        </section>
        
        <h3>Beschrijving van de attributen en relaties</h3>
        <section id="properties">
            <h4><a href="#voorbeeldfeaturetype">Voorbeeld featuretype</a></h4>
            <section id="voorbeeldfeaturetypeproperties">
                <!-- tabel met de metadata voor properties. 
                         In veld Domein Type een href naar het anchor van het datatype indien gespecificeerd in dit model-->
            </section>
        </section>
        <h3>Beschrijving van de datatypen en lijsten</h3>
        <section id="datatypes">
            <!-- Hier komen de: 
                      ...
                      - unions
                      - primitive datatypes -->
            <h4>Voorbeeld codelist</h4>
            <section id="voorbeeldcodelist">
                <!-- tabel met de metadata voor codelists -->
            </section>
            <h4>Voorbeeld enumeratielijst</h4>
            <section id="voorbeeldenum">
                <table>
                    <tr>
                        <th>Naam</th>
                        <td>...</td>
    ...
    <xsl:template match="imvert:class[imvert:stereotype/@id = ('stereotype-name-objecttype')]">
        ...
        <h4>
            <xsl:value-of select="concat('FeatureType ', imvert:name/@original)"/>
        </h4>
        <table>
            <tbody>
    ...
    <xsl:template match="imvert:class[imvert:stereotype/@id = ('stereotype-name-relatieklasse')]">
        <h2>
            <xsl:value-of select="concat('AssociationClass ', imvert:name/@original)"/>
        </h2>
        <table>
            <tbody>
                <xsl:sequence select="imf:label-waarde('Naam',imvert:name/@original)"/>
   ...
   <xsl:template match="imvert:attribute" mode="detail">
    ...
    <xsl:template match="imvert:association" mode="detail">
        <xsl:variable name="construct" select="../.."/>
        <xsl:variable name="defining-class" select="if (exists(imvert:type-id)) then imf:get-construct-by-id(imvert:type-id) else ()"/>
        <xsl:variable name="is-identifying" select="imvert:target/imvert:stereotype/@id = ('stereotype-name-composite-id')"/>
        <h4>
    ...
    <xsl:function name="imf:get-section">
        ...
        <xsl:variable name="enums" as="element()*">
            <xsl:apply-templates select="$package/imvert:class[imvert:stereotype/@id = ('stereotype-name-enumeration')]">
        ...
        <xsl:variable name="sections" as="element()*">
            <xsl:choose>
                <xsl:when test="not($detail) and ($stereotype/@id = 'stereotype-name-enumeration') and exists($enums)">
                    <table>
                        <tbody>
                            <tr>
                ...
                <xsl:when test="$detail">
                    <xsl:apply-templates select="$package/imvert:class[imvert:stereotype/@id = ($stereotype)]" mode="detail">
                        <xsl:sort select="imvert:name"/>
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:apply-templates select="$package/imvert:class[imvert:stereotype/@id = ($stereotype)]">
                        <xsl:sort select="imvert:name"/>
                    </xsl:apply-templates>
                </xsl:otherwise>
            </xsl:choose>
