<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="3.0"
    ...
    <!-- 
        deze stylesheet moet geheel worden omgezet naar MIM-format gebaseerde input 
    -->
    ...
    <xsl:template match="/imvert:packages">
        <xsl:variable name="body" as="element()">
            <ep:group 
                ...
                <ep:parameters>
                    <xsl:sequence select="imf:set-parameter('group-type','model')"/>
                ...
                <ep:release>
                    <xsl:value-of select="imvert:release"/>
                </ep:release>
    ...
    <xsl:template match="imvert:package[imvert:stereotype/@id = 'stereotype-name-domain-package']">
        <ep:group>
            ...
            <ep:parameters>
                <xsl:sequence select="imf:set-parameter('group-type','domain')"/>
    ...
    <xsl:function name="imf:get-note-value" as="item()*">
        ...
        <ep:text>
            <xsl:choose>
                <xsl:when test="$note-field/self::xhtml:body">
                    <xsl:sequence select="pack:strip($note-field/node())"/>
                </xsl:when>
